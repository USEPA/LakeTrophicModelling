...
```{r rand_forest_runs , eval=TRUE, include=FALSE, echo=FALSE, cache=FALSE}
################################################################################
...
all_vs <- varsel_regression_rf(all_dat$LogCHLA,all_dat[,names(all_dat)!="LogCHLA"],
                     ntree=5000,prog=T, importance = TRUE)  
...
#all_vars_select <- c("TURB", "PTL", "NTL", "ELEV_PT", "TOC", "NPratio", 
#                     "WSA_ECO9", "AlbersX", "AlbersY", "NO3_NO2","ORGION", 
#                     "DOC", "DEPTHMAX", "K", "TmeanW", "PH_FIELD", "DDs45", 
#                     "DevOpenPer_3000m", "CL", "DO2_2M")
...
#gis_vars_select <- c("WSA_ECO9","CropsPer_3000m", "ELEV_PT", "AlbersY", 
#                     "EvergreenPer_3000m","MeanDepthCorrect", "AlbersX", 
#                     "WoodyWetPer_3000m", "HerbWetPer_3000m", "DDs45", 
#                     "MaxDepthCorrect", "DeciduousPer_3000m", 
#                     "DevOpenPer_3000m", "GrassPer_3000m", "BASINAREA")
...
```{r class_predict_probability, eval=TRUE, echo=FALSE, cache=TRUE, warning=FALSE, message = FALSE}
...
all_rf_chla <- data.frame(nla_id = names(all_rf$predicted), chla_pred = 10^all_rf$predicted)
all_rf_chla <- left_join(all_rf_chla, ltmData, by = c("nla_id" = "NLA_ID"))
all_rf_chla <- left_join(all_rf_chla, all_rf_ts_prob)
all_rf_out <- select(all_rf_chla, nla_id, chla_pred, chla_obs = CHLA, 
                     ts_pred_train = pred_class, ts_pred_oob = pred_class_oob, 
                     ts_obs = obs_class, oligo_prob = oligo, meso_prob = meso, 
                     eu_prob = eu, hyper_prob = hyper, 
                     oligo_prob_oob = oligo_oob, meso_prob_oob = meso_oob, 
                     eu_prob_oob = eu_oob, hyper_prob_oob = hyper_oob)
all_rf_out <- mutate(all_rf_out, across(where(is.numeric), round, 3))
write.csv(all_rf_out, "../inst/extdata/all_rf_out.csv", row.names = FALSE)

gis_rf_chla <- data.frame(nla_id = names(gis_rf$predicted), chla_pred = 10^gis_rf$predicted)
gis_rf_chla <- left_join(gis_rf_chla, ltmData, by = c("nla_id" = "NLA_ID"))
gis_rf_chla <- left_join(gis_rf_chla, gis_rf_ts_prob)
gis_rf_out <- select(gis_rf_chla, nla_id, chla_pred, chla_obs = CHLA, 
                     ts_pred_train = pred_class, ts_pred_oob = pred_class_oob, 
                     ts_obs = obs_class, oligo_prob = oligo, meso_prob = meso, 
                     eu_prob = eu, hyper_prob = hyper, 
                     oligo_prob_oob = oligo_oob, meso_prob_oob = meso_oob, 
                     eu_prob_oob = eu_oob, hyper_prob_oob = hyper_oob)
gis_rf_out <- mutate(gis_rf_out, across(where(is.numeric), round, 3))
write.csv(gis_rf_out, "../inst/extdata/gis_rf_out.csv", row.names = FALSE)
```
...
ts_prob_map(state,lakes_dd,gis_rf_ts_prob, original = TRUE)
```

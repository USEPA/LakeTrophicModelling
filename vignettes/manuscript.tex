...
we build, assess, and compare two random forest models of chlorophyll
\emph{a} 1) \emph{in situ} and universally available GIS data and then
2) universally available GIS data only. Second, we convert the
chlorophyll \emph{a} estimates, for both models, to trophic state and
assess prediction accuracy and uncertainty. Third, we examined the
important predictors for both models.

To promote repeatable research and open access to data and code, the
analysis code, and the data used in the models are available as an R
package from \url{https://github.com/USEPA/LakeTrophicModelling}. This
will allow people with basic R skills to reproduce the paper with the
figures and tables, and rerun or modify the analyses.
...
chlorophyll \emph{a} with two sets of predictors (Liaw and Wiener 2002).
The first included \emph{in situ} and universally available GIS
predictors. We refer to this as the ``All variables'' model. For the
second model we used just the univerally available data (i.e., no
\emph{in situ} information). This is referred to as the ``GIS only''
model. A list of the full suite of variables tested is in Appendix 1.
Our separation of predictors was chosen so that we could highlight the
...
the model predictions when converted to trophic states classes (Table
\ref{tab:trophicStateTable}). We assessed the classifcations via a
...
calculated the total accuracy (i.e., percent correctly predicted) and
the kappa coefficient, which takes into account the error expected by
chance alone (i.e., the off diagonal values of the matrix) (Cohen 1960,
Hubert and Arabie 1985). The kappa coefficient can range from -1 to 1
with 0 equalling the agreement expected by chance alone. Values greater
than 0 represent agreement greater than would be expected by chance,
with values greater than 0.61 considered ``substantial'' agreement
(Landis and Koch 1977). Negative values are rare and would indicate no
agreement between the predicted and observed values. Additonally, random
forest builds each tree on bootstrapped, random subsets of the original
data, thus, a separate independent validation dataset is not required
and random forest error estimates are expected to be unbiased (Breiman
2001).
...
(Strobl et al. 2007), thus, we used percent increase in mean squared
error to assess variable importance. Lastly, partial dependence plots
provide a mechanism to examine the partial relationship between
individual variables and the response variable (Jones and Linder 2015).
We examined these plots for the top variables as assigned by percent
...
the distribution of prediction probabilities is skewed left indicating a
bias towards higher prediction probabilities and therefore greater
accuracy (Figure \ref{fig:pred_prob_dist}). While these patterns are not
strong, they suggest that with slight improvements in the ``GIS only''
model we could skew the prediction probabilites futher left and easily
improve the overall accuracy of the model. This could be done using
modeled, national estimates of nutrient loads (Moore et al. 2011, e.g.,
Milstead et al. 2013).
...
realated to cyanobacteria (e.g., toxin presence). Others have seen these
...
produced by some cyanobacteria) criteria exceedences. Using these same
...
\toprule
Variable Names & Description & Source & NA\tabularnewline
\midrule
\endhead
18 & AlbersX & Longitude (Albers meters) & GIS\tabularnewline
19 & AlbersY & Latitude (Albers meters) & GIS\tabularnewline
8 & BarrenPer\_3000m & Percent Barren & GIS\tabularnewline
25 & BASINAREA & Watershed Area (sq. meters) & GIS\tabularnewline
15 & CropsPer\_3000m & Percent Cropland & GIS\tabularnewline
60 & DDs45 & Growing Degree Days (Days) & GIS\tabularnewline
9 & DeciduousPer\_3000m & Percent Decidous Forest & GIS\tabularnewline
7 & DevHighPer\_3000m & Percent High Intensity Development &
GIS\tabularnewline
5 & DevLowPer\_3000m & Percent Low Intensity Development &
GIS\tabularnewline
6 & DevMedPer\_3000m & Percent Medium Intensity Development &
GIS\tabularnewline
4 & DevOpenPer\_3000m & Percent Developed Open Space &
GIS\tabularnewline
27 & ELEV\_PT & Elevation (meters) & GIS\tabularnewline
10 & EvergreenPer\_3000m & Percent Evergreen Forest & GIS\tabularnewline
66 & FetchE & Fetch from East (m) & GIS\tabularnewline
64 & FetchN & Fetch from North (m) & GIS\tabularnewline
65 & FetchNE & Fetch form Northeast (m) & GIS\tabularnewline
67 & FetchSE & Fetch from Southeast (m) & GIS\tabularnewline
13 & GrassPer\_3000m & Percent Grassland & GIS\tabularnewline
17 & HerbWetPer\_3000m & Percent Herbaceuos Wetland & GIS\tabularnewline
3 & IceSnowPer\_3000m & Percent Ice/Snow & GIS\tabularnewline
20 & LakeArea & Lake Surface Area (sq. meters) & GIS\tabularnewline
21 & LakePerim & Lake Perimeter (meters) & GIS\tabularnewline
68 & MaxDepthCorrect & Estimated Maximum Lake Depth (m) &
GIS\tabularnewline
61 & MaxLength & Maximum Lake Length (m) & GIS\tabularnewline
62 & MaxWidth & Maximum Lake Width (m) & GIS\tabularnewline
70 & MeanDepthCorrect & Estimated Mean Lake Depth (m) &
GIS\tabularnewline
63 & MeanWidth & Mean Lake Width (m) & GIS\tabularnewline
11 & MixedForPer\_3000m & Percent Mixed Forest & GIS\tabularnewline
14 & PasturePer\_3000m & Percent Pasture & GIS\tabularnewline
1 & PercentImperv\_3000m & Percent Impervious & GIS\tabularnewline
22 & ShoreDevel & Shoreline Development Index & GIS\tabularnewline
12 & ShrubPer\_3000m & Percent Shrub/Scrub & GIS\tabularnewline
69 & VolumeCorrect & Estimated Lake Volume (cubic meters) &
GIS\tabularnewline
2 & WaterPer\_3000m & Percent Water & GIS\tabularnewline
16 & WoodyWetPer\_3000m & Percent Woody Wetland & GIS\tabularnewline
24 & WSA\_ECO9 & Ecoregion & GIS\tabularnewline
31 & ANC & Acid Neutralizing Capacity (ueq/L) & Water
Quality\tabularnewline
53 & ANDEF2 & Anion Deficit (ueq/L) & Water Quality\tabularnewline
52 & ANSUM2 & Sum of Anions using ANC (ueq/L) & Water
Quality\tabularnewline
55 & BALANCE2 & Ion Balance (\%) & Water Quality\tabularnewline
42 & CA & Calcium (ueq/L) & Water Quality\tabularnewline
51 & CATSUM & Sum of Cations (ueq/L) & Water Quality\tabularnewline
39 & CL & Chloride (ueq/L) & Water Quality\tabularnewline
46 & COLOR & Color (PCU) & Water Quality\tabularnewline
57 & CONCAL2 & Calculated Conductivity (uS/cm) & Water
Quality\tabularnewline
30 & COND & Conductivity (uS/cm) & Water Quality\tabularnewline
58 & CONDHO2 & D-H-O Calculated Conductivity (uS/cm) & Water
Quality\tabularnewline
23 & DATE\_COL & Date Samples Collected & Water Quality\tabularnewline
26 & DEPTHMAX & Maximum Depth (meters) & Water Quality\tabularnewline
28 & DO2\_2M & Dissolved Oxygen (mg/L) & Water Quality\tabularnewline
34 & DOC & Dissolved Organic Carbon (mg/L) & Water
Quality\tabularnewline
48 & H & Hydrogen Ions (ueq/L) & Water Quality\tabularnewline
45 & K & Potassium (ueq/L) & Water Quality\tabularnewline
43 & MG & Magnesium (ueq/L) & Water Quality\tabularnewline
44 & Na & Sodium (ueq/L) & Water Quality\tabularnewline
35 & NH4 & Ammonium (mg/L) & Water Quality\tabularnewline
50 & NH4ION & Calculated Ammonium (ueq/L) & Water Quality\tabularnewline
40 & NO3 & Nitrate (ueq/L) & Water Quality\tabularnewline
36 & NO3\_NO2 & Nitrate/Nitrite (mg N/L) & Water Quality\tabularnewline
71 & NPratio & Nitrogen:Phophorus Ratio & Water Quality\tabularnewline
37 & NTL & Total Nitrogen (\(\\mu\)g/L) & Water Quality\tabularnewline
49 & OH & Hydroxide (ueq/L) & Water Quality\tabularnewline
56 & ORGION & Estimated Organic Anions (ueq/L) & Water
Quality\tabularnewline
29 & PH\_FIELD & pH & Water Quality\tabularnewline
38 & PTL & Total Phosphorus (\(\\mu\)g/L) & Water Quality\tabularnewline
47 & SIO2 & Silica (mg/L) & Water Quality\tabularnewline
41 & SO4 & Sulfate (ueq/L) & Water Quality\tabularnewline
54 & SOBC & Sum of Base Cation (ueq/L) & Water Quality\tabularnewline
59 & TmeanW & Mean Profile Water Temperature (C ) & Water
Quality\tabularnewline
33 & TOC & Total Organic Carbon (mg/L) & Water Quality\tabularnewline
32 & TURB & Turbidity (NTU) & Water Quality\tabularnewline
\bottomrule
\end{longtable}

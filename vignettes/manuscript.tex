...
coefficient of 0.5 (Tables \ref{tab:Confusion_GIS_4} and
\ref{tab:Conusion_GIS-2}).
...
with 15 variables (Figure \ref{fig:gis_varsel_figure}). The five most
important variables were ecoregion, percent cropland, elevation,
latitude, and percent evegreen forest. (Figure
\ref{fig:GIS_Importance}).

{[}JEFF START HERE{]} \#\#Per Lake Probability As expected, the ``All
Variables'' model provides higher total accuracy than the ``GIS Only''
model with a median prediction probability of 0.81 and 0.72 (Figure
\ref{fig:prob_cdf}). Yet, the two distributions are {[}TEST FOR DIFF IN
DISTRIBUTIONS{]}

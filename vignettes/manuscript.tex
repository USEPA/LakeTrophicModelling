...
approaches. To predict trophic state, we first model the continous
values, chlorophyll \emph{a} in this case, then classify the resultant
predictions. The first model (i.e.~the full model) estimates chlorophyll
\emph{a} with both \emph{in situ} and universally available data and the
mean squared error and adjusted R\textsuperscript{2} of this model was
0.09 and 0.8. The second model (i.e.~the GIS only models) uses
universally available GIS data only for models and the mean squared
error was 0.22 and the adjusted R\textsuperscript{2} was 0.48. The
accuracy of various trophic state classifications derived from the
chlorophyll \emph{a} predictions ranged from 69\% to 87\% for the full
model and from 49\% to 75\% for the GIS only model. The source code and

...
There was a great deal of agreement on the important variables for each set of models.  In line with past predictive modeling of cyanobacteria abundance, the in situ models consistently select turbidity, total nitrogen, total phosphrous, and N:P ratios as important variables [@downing2001predicting].   While there is variation in the response of cyanobacteria to changes in relative nutrient concentrations, the general pattern suggests that limiting nutrients have considerable impact once amounts increase beyond expected levels.  

The mechanistic role of turbidity on lake trophic state is more complex. Turbidity is related to trophic state and chlorophyll *a* in two ways.  First, light availability in turbid waters is lower than in clear waters.  This would suggest a negative relationship between turbidity and chlorophyll *a*.  Second, chlorophyll *a* can also be a component of turbidity and lakes with higher chlorophyll *a* concentrations will also be more turbid.  This would be a cause for concern with linear models; however, with tree-based modelling approaches      As lake become more eutrophic, they have more “stuff” in them and are thus more turbid, clearly increasing chlorophyll a levels.   

Our GIS models are capturing the large scale trophic status gradient of lakes across the United States.  We reliably see latitude and longitude and ecoregion selected as important variables.  It is also arguably other variables selected as important which are proxies for the lake trophic status gradient (e.g. elevation and growing degree days).   Elevation clearly plays a more nuanced role in lake trophic status.  However, the relative continental position of mountains in the United States is the spatial inverse to the distribution of agricultural lands.  Both percent cropland and evergreen forest coverage could both be also illustrating this gradient, but perhaps illustrating potential drivers of lake trophic status.  

...
##Model Details
Using both the `varSelRF` and `randomForest` R packages we ran models for six sets of variables and trophic state classifications [@diaz-uriarte2010varSelRF; @liaw2002randomForest].  These included three different combinations of the Chlorophyll *a* trophic states as the dependent variables and using all variables (*in situ* and GIS variables) or the GIS only variables (i.e. no *in situ* information) as the independent variables in the random forest.  A listing of all considered variables is in Appendix 1.  Trophic state was defined using the NLA chlorophyll *a* trophic state cut offs and the three combinations of trophic state were used to highlight the possible error caused by misclassification of adjacent classes, such as mesotrophic and eutrophic (Table \ref{tab:trophicStateTable}).  Lastly, we used only complete cases (i.e missing data were removed) so the total number of observations varied between models. 
...
```{r Confusion_Model1,results="asis",echo=FALSE, tab.cap="Random Forest confusion matrix for Model 1. Columns show predicted values and rows show observed values.  Agreement indicated on diagonal. \\label{tab:Confusion_Model1}",cache=FALSE}
kable(round(all_ts4_rf$confusion,2),row.names=T)
```

\newpage

```{r Confusion_Model2,results="asis",echo=FALSE, tab.cap="Random Forest confusion matrix for Model 2. Columns show predicted values and rows show observed values.  Agreement indicated on diagonal.  \\label{tab:Confusion_Model2}",cache=FALSE}
kable(round(all_ts3_rf$confusion,2),row.names=T)
```

\newpage

```{r Confusion_Model3,results="asis",echo=FALSE, tab.cap="Random Forest confusion matrix for Model 3. Columns show predicted values and rows show observed values.  Agreement indicated on diagonal. \\label{tab:Confusion_Model3}",cache=FALSE}
kable(round(all_ts2_rf$confusion,2),row.names=T)
```

\newpage

```{r Confusion_Model4,results="asis",echo=FALSE, tab.cap="Random Forest confusion matrix for Model 4. Columns show predicted values and rows show observed values.  Agreement indicated on diagonal. \\label{tab:Confusion_Model4}",cache=FALSE }
kable(round(gis_ts4_rf$confusion,2),row.names=T)
```

\newpage

```{r Conufsion_Model5,results="asis",echo=FALSE, tab.cap="Random Forest confusion matrix for Model 5. Columns show predicted values and rows show observed values.  Agreement indicated on diagonal. \\label{tab:Confusion_Model5}",cache=FALSE}
kable(round(gis_ts3_rf$confusion,2),row.names=T)
```

\newpage

```{r Confusion_Model6,results="asis",echo=FALSE, tab.cap="Random forest confusion matrix for Model 6. Columns show predicted values and rows show observed values.  Agreement indicated on diagonal. \\label{tab:Confusion_Model6}",cache=FALSE}
kable(round(gis_ts2_rf$confusion,2),row.names=T)
```

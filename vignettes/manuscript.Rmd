...
##Trophic State Probabilities
Lakes with more certain predictions (i.e. higher prediction probabilities) were more accurately predicted (Figure \ref{fig:cond_prob_fig}).  The fact that the difference in accuracy between the models decreased as certainty in the prediction increased suggests that, even for models with lower overall accuracy (i.e. the "GIS Only" model) may, these models may have acceptable accuracy for many individual cases (Table \ref{tab:cond_prob_tab}).  
...
[NEED TO ADD A SPECIFIC CONCLUSION THAT GIS IS FINE]

...
#Model 1: All Variables
  ...
  cf_dat <- na.omit(data.frame(ltmData[predictors_all],CHLA=ltmData$CHLA))
  cf_dat$DATE_COL <- factor(cf_dat$DATE_COL)
  all_cf <- cforest(log10(CHLA)~.,data=cf_dat,
                    control=cforest_control(ntree=100,mtry=ncol(cf_dat)/3))
  all_cf_pred <- predict(all_cf)
  par(mfrow=c(1,2))
  plot(log10(cf_dat$CHLA),all_cf_pred)
  plot(log10(cf_dat$CHLA),all_rf$pred)
  glance(lm(all_cf_pred~log10(cf_dat$CHLA)))
  glance(lm(all_rf$pred~log10(cf_dat$CHLA)))
################################################################################

...
all_ts4_pred <- cut(all_pred,ts_4_brks,ts_4_cats,include.lowest=T)
...
```{r class_predict_probability}
#Per tree chl a predictions - All Variables
all_rf_alltree_predict <- predict(all_rf,newdata=all_dat,predict.all=TRUE)
all_rf_ts_prob <- apply(all_rf_alltree_predict$individual,2,
                        function(x) cut(x,ts_4_brks,ts_4_cats)) 
all_rf_ts_prob <- apply(all_rf_ts_prob,1, function(x)
                        table(factor(x,levels=c("oligo","meso","eu","hyper"),
                                     ordered=T))/5000)
all_rf_ts_prob <- data.frame(t(all_rf_ts_prob))  

gis_rf_alltree_predict <- predict(gis_rf,newdata=gis_dat,predict.all=TRUE)
gis_rf_ts_prob <- apply(gis_rf_alltree_predict$individual,2,
                        function(x) cut(x,ts_4_brks,ts_4_cats)) 
gis_rf_ts_prob <- apply(gis_rf_ts_prob,1, function(x)
                        table(factor(x,levels=c("oligo","meso","eu","hyper"),
                                     ordered=T))/5000)
gis_rf_ts_prob <- data.frame(t(gis_rf_ts_prob))  
...
#lakeVotes<-data.frame(COMID=getLakeIDs(ltmData[all_vars_select],ltmData$TS_CHLA_4,ltmData$Comid),lakeVotes)

...
[MOVE SOME OF THIS TO METHODS, CHECK FOR OVERLAP IN RESULTS, DETERMINE WHAT SHOULD GO IN DISCUSS]

##Trophic State Probabilities
As expected, lakes with higher maximum vote probabilities were more accurately predicted (Figure \ref{fig:condProbFig}).  The increasing trend suggests that even for models with lower overall accuracy there can also be a large number of individual cases that are predicted with high accuracy.
...
```{r all_partial_dependence, eval=TRUE, echo=FALSE, fig.cap="\"All Variables\" Partial dependence plots for the top 5 most important variables. \\label{fig:all_partial_dependence}",cache=TRUE,fig.height=8}
par(mfrow=c(3,2))
interpretR::parDepPlot("TURB",all_rf,data=all_dat,robust=T,ci=T,u.quant=0.975,l.quant=0.025,n.pt = 100)
interpretR::parDepPlot("NTL",all_rf,data=all_dat,robust=T,ci=T,u.quant=0.975,l.quant=0.025, n.pt = 100)
interpretR::parDepPlot("PTL",all_rf,data=all_dat,robust=T,ci=T,u.quant=0.975,l.quant=0.025, n.pt = 100)
interpretR::parDepPlot("ELEV_PT",all_rf,data=all_dat,robust=T,ci=T,u.quant=0.975,l.quant=0.025,n.pt = 100)
interpretR::parDepPlot("TOC",all_rf,data=all_dat,robust=T,ci=T,u.quant=0.975,l.quant=0.025,n.pt = 100)
par(mfrow=c(1,1))
```
...
```{r gis_partial_dependence,eval=TRUE, echo=FALSE, fig.cap="\"GIS Only Variables\" Partial dependence plots for the top 5 most important variables. \\label{fig:gis_partial_dependence}",cache=TRUE,fig.height=8}
par(mfrow=c(3,2))
interpretR::parDepPlot("WSA_ECO9",gis_rf,data=gis_dat,robust=T,ci=T,u.quant=0.975,l.quant=0.025,n.pt = 100)
interpretR::parDepPlot("CropsPer_3000m",gis_rf,data=gis_dat,robust=T,ci=T,u.quant=0.975,l.quant=0.025, n.pt = 100)
interpretR::parDepPlot("ELEV_PT",gis_rf,data=gis_dat,robust=T,ci=T,u.quant=0.975,l.quant=0.025, n.pt = 100)
interpretR::parDepPlot("AlbersY",gis_rf,data=gis_dat,robust=T,ci=T,u.quant=0.975,l.quant=0.025,n.pt = 100)
interpretR::parDepPlot("EvergreenPer_3000m",gis_rf,data=gis_dat,robust=T,ci=T,u.quant=0.975,l.quant=0.025,n.pt = 100)
par(mfrow=c(1,1))
```
...
```{r gis_probability_map, eval=TRUE, results="asis",echo=FALSE, fig.cap="Maps of prediction probabilities for each of the four chlorophyll \\textit{a} trophic states  \\label{fig:gis_probability_map}",cache=FALSE,fig.height=10}
state<-map_data('state')
lakes_alb<-data.frame(ltmData[["AlbersX"]],ltmData[["AlbersY"]],
                     ltmData[["NLA_ID"]])
p4s<-"+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=37.5 +lon_0=-96 +x_0=0 +y_0=0 +ellps=GRS80 +datum=NAD83 +units=m +no_defs" 

...
##Models
Accuracy for the models built with all predictors ranged from `r round(min(acc_results[1:3,2]),2)` to `r round(max(acc_results[1:3,2]),2)` and the kappa coefficient had a minimum value of `r round(min(acc_results[1:3,3]),2)` and maximum of `r round(max(acc_results[1:3,3]),2)`.  The GIS only models had a total accuracy between `r round(min(acc_results[4:6,2]),2)` and `r round(max(acc_results[4:6,2]),2)` and kappa coefficient between `r round(min(acc_results[4:6,3]),2)` and `r round(max(acc_results[4:6,3]),2)`.  The importance of variables for the models including the *in situ* data were fairly stable while there was considerably more variation in variable importance for the three different GIS only models.  Additionally, as each model contained a difference subset of variables and since we removed observations with missing data, the total number of lakes varied across the models.  Details for each model are discussed below.

...
#figure and table numbering (modified from https://github.com/yihui/knitr-examples/blob/master/070-caption-num.Rmd)
cap_num<-local({
  i_f<-0
  i_t<-0
  function(caption,type){
    if(type=="figure"){
      i_f<-i_f+1
      cap<-paste('Figure ', i_f,': ',caption, sep='')
      } else{
      i_t<-i_t+1
      cap<-paste('Table ', i_t,': ',caption, sep='')
      }
    }
  })
...
```{r Table1, results='asis', echo=FALSE,fig.cap=cap_num('Variable and variable definitions for all possible predcitors','table')}

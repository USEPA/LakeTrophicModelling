...
The "All Variables" model provides more certain model prediction than the "GIS Only" model with a median prediction probability of `r round(median(apply(all_rf_ts_prob[1:4],1,max)),2)` versus `r round(median(apply(gis_rf_ts_prob[1:4],1,max)),2)` (Figure \ref{fig:prob_cdf}).  
Additionally, total accuracy of the predictions is a function of this uncertainty.  Lakes with more certain predictions are more accurately classified (Figure \ref{fig:cond_prob_fig}).  For both models, when prediction probabilites are approximate 0.7 or higher, the models have an accuracy of 100%.  This represents `r round(sum(all_rf_ts_prob$max>0.7)/nrow(all_dat),2)*100`% of the lakes for the "All Variables" model and `r round(sum(gis_rf_ts_prob$max>0.7)/nrow(all_dat),2)*100`% of the lakes for the "GIS Only" model.

...
#Model 1: All Variables
  ...
  all_cf_dat <- na.omit(data.frame(ltmData[predictors_all],CHLA=ltmData$CHLA))
  all_cf_dat$DATE_COL <- factor(all_cf_dat$DATE_COL)
  all_cf <- cforest(log10(CHLA)~.,data=all_cf_dat,
                    control=cforest_unbiased(ntree=100,mtry=ncol(all_cf_dat)/3))
  all_cf_pred <- predict(all_cf)
  all_cf_imp <- varimp(all_cf)
  all_cf_pd<-partial_dependence(all_cf,var="NTL")
################################################################################
#Model 2: GIS Only Variables
  ...
  #gis_rf<-runRandomForest(ltmData[gis_vars],ltmData$TS_CHLA_4, 
                              ntree=10000,importance=TRUE,proximity=TRUE)
  gis_cf_dat <- na.omit(data.frame(ltmData[predictors_gis],CHLA=ltmData$CHLA))
  gis_cf <- cforest(log10(CHLA)~.,data=gis_cf_dat,
                    control=cforest_unbiased(ntree=100,mtry=ncol(gis_cf_dat)/3))

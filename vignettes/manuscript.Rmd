...
if(!require(edarf)){
  devtools::install_github("zmjones/edarf",quick=TRUE)
}
...
#Model 1: All Variables
  #Variable Selction
  #all_vsrf<-iterVarSelRF(ltmData[predictors_all],ltmData$CHLA,numCore=4,2,
  #      								ntree=10000,ntreeIterat=5000,vars.drop.frac=NULL,
  #  											vars.drop.num=1,outStr="all_ts4",time=TRUE)

  #Random Forest
  #all_vars<-unique(unlist(all_vsrf[1:100]))
  #all_rf<-runRandomForest(ltmData[predictors_all],log10(ltmData$CHLA), 
  #                        ntree=1000,importance=TRUE,proximity=TRUE)
  #                        #keep.forest=TRUE,keep.inbag=TRUE)
  predictors_all <- predictors_all[predictors_all!="DATE_COL"]
  all_rf_dat <- data.frame(ltmData[predictors_all],LogCHLA=log10(ltmData$CHLA))
  all_rf_dat <- all_rf_dat[complete.cases(all_rf_dat),]
  all_rf<-randomForest(LogCHLA~.,data=all_rf_dat, 
                         ntree=10000,importance=TRUE,proximity=TRUE,
                          keep.forest=TRUE,keep.inbag=TRUE)
  
  #all_cf_imp <- varimp(all_cf)
  #all_cf_pd <- partial_dependence(all_cf,var="NTL",cutoff=30L)
  #all_cf_stats <- cforestStats(all_cf)
  #x<-data.frame(importance = all_cf_imp,parameters = names(all_cf_imp))
  #plot(x)
  #all_cf.vsurf <- VSURF(all_cf_dat[,names(all_cf_dat)!="LogCHLA"],all_cf_dat$LogCHLA, ntree = 100, nfor.thres = 25,
  #                  nfor.interp = 15, nfor.pred = 15)
################################################################################
#Model 2: GIS Only Variables
  #Variable Selction
  #gis_vsrf<-iterVarSelRF(ltmData[predictors_gis],ltmData$TS_CHLA_4,numCore=4,100,
  #        							ntree=10000,ntreeIterat=5000,vars.drop.frac=NULL,
  #											vars.drop.num=1,outStr="gis_ts4",time=TRUE)
 
  #Random Forest
  #gis_vars<-unique(unlist(gis_vsrf[1:100]))
  gis_rf_dat <- data.frame(ltmData[predictors_gis],LogCHLA=log10(ltmData$CHLA))
  gis_rf_dat <- gis_rf_dat[complete.cases(gis_rf_dat),]
  gis_rf<-randomForest(LogCHLA~.,data=gis_rf_dat, 
                         ntree=10000,importance=TRUE,proximity=TRUE,
                          keep.forest=TRUE,keep.inbag=TRUE)
  #gis_cf <- cforest(LogCHLA~.,data=gis_cf_dat,
  #                  control=cforest_unbiased(ntree=1000,mtry=ncol(gis_cf_dat)/3))
  #gis_cf_pred <- predict(all_cf)
  #gis_cf_imp <- varimp(gis_cf)
  #gis_cf_pd<-partial_dependence(gis_cf,var="NTL",cutoff=30L)
  #gis_cf_stats<-cforestStats(gis_cf)
```
...
Our research goals were to explore the utility of a widely used data mining algorithm, random forests, in the modelling of lake trophic state. Further, we hoped to examine the utility of these models when built with only ubiquitous GIS data, which allows estimation of trophic state for all lakes in the United States. We were able to successfully predict a variety of trophic state classes.  With the GIS only models our total accuracy ranged from `r 100*round(min(acc_results$total_accuracy[4:6]),2)`% to `r 100*round(max(acc_results$total_accuracy[4:6]),2)`%, and with the full suite of data our model accuracy had a minimum accuracy of `r 100*round(min(acc_results$total_accuracy[1:3]),2)`% and maximum accuracy of `r 100*round(max(acc_results$total_accuracy[1:3]),2)`%.  
...
```{r fig1_nlaMap, echo=FALSE, fig.cap="Map of the distribution of National Lakes Assesment Sampling locations\\label{fig:nlaMap}",cache=FALSE}
...
```{r fig2_Importance_Model1,results="asis",echo=FALSE, fig.cap="Importance plot for Model 1, shows mean descrease gini and proportion of times a variable was selected for inclusion in the model.  Higher values of mean decrease gini indicate higher importance.  \\label{fig:Importance_Model1}",cache=FALSE}
...
```{r fig3_Importance_Model2,results="asis",echo=FALSE, fig.cap="Importance plot for Model 2, shows mean descrease gini and proportion of times a variable was selected for inclusion in the model.  Higher values of mean decrease gini indicate higher importance. \\label{fig:Importance_Model2}",cache=FALSE}
...
```{r fig4_Importance_Model3,results="asis",echo=FALSE, fig.cap="Importance plot for Model 3, shows mean descrease gini and proportion of times a variable was selected for inclusion in the model.  Higher values of mean decrease gini indicate higher importance. \\label{fig:Importance_Model3}",cache=FALSE}
...
```{r fig5_Importance_Model4,results="asis",echo=FALSE, fig.cap="Importance plot for Model 4, shows mean descrease gini and proportion of times a variable was selected for inclusion in the model.  Higher values of mean decrease gini indicate higher importance. \\label{fig:Importance_Model4}",cache=FALSE}
...
```{r fig6_Importance_Model5,results="asis",echo=FALSE, fig.cap="Importance plot for Model 5, shows mean descrease gini and proportion of times a variable was selected for inclusion in the model.  Higher values of mean decrease gini indicate higher importance. \\label{fig:Importance_Model5}",cache=FALSE}
...
```{r fig7_Importance_Model6,results="asis",echo=FALSE, fig.cap="Importance plot for Model 6, shows mean descrease gini and proportion of times a variable was selected for inclusion in the model.  Higher values of mean decrease gini indicate higher importance. \\label{fig:Importance_Model6}",cache=FALSE}
...
```{r fig8_condProbFig,echo=FALSE, fig.width=7, fig.cap="Comparison of certainity of trophic state prediction and total accuracy\\label{fig:condProbFig}",cache=FALSE}
condAccuracy(gis_ts4_rf,xImpair=0,R=1,xlab="Maximum Vote Probability")
```

\newpage

```{r fig9_ts_4_cyano_cdf, echo=FALSE, fig.width=7, fig.cap="Cumulative distribution function of cyanobacetria abundance for 4 trophic state classes\\label{fig:ts_4_cyano_cdf}",cache=FALSE}
...
```{r fig10_ts_3_cyano_cdf, echo=FALSE, fig.width=7, fig.cap="Cumulative distribution function of cyanobacetria abundance for 3 trophic state classes\\label{fig:ts_3_cyano_cdf}",cache=FALSE }
...
```{r fig11_ts_2_cyano_cdf, echo=FALSE, fig.width=7, fig.cap="Cumulative distribution function of cyanobacetria abundance for 2 trophic state classes\\label{fig:ts_2_cyano_cdf}",cache=FALSE}
...
```{r fig12_scatterplot,echo=FALSE, fig.width=7, fig.cap="Cholorphyll *a* and cyanobacteria abundance scatterplot\\label{fig:scatterplot}",cache=FALSE}

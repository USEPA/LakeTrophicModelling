...
The model built with all predictors had a mean squared error of `r round(all_rf$mse,2)` and and R\textsuperscript{2} of `r round(all_rf$rsq,2)`.  The accuracy of the four trophic states was `r round(all_ts4_total_acc,2)*100`% and the kappa coefficient was `r round(all_ts4_kappa,2)` (Table \ref{tab:Confusion_All_4}). For the combined, two trophic states it was `r round(all_ts2_total_acc,2)` and `r round(all_ts2_kappa,2)*100`% for accuracy and the kappa coefficient, respectively (Table \ref{tab:Confusion_All_2}).  

The reduced, all variables model was built using `r nrow(all_dat)` total observations. The variable selection process identified 20 variables (Figure \ref{fig:all_varsel_figure}).  
<!-- Jeff Start Here PROBLEM IN THE ABOVE NOT SINGLE VALUE SOMEWHERE--!>
The  most important variables were ecoregion, growing degree days, and percent evergreen. (Figure \ref{fig:All_Importance}).  MORE HERE.

###Per Lake Probability
Historgram of Binned max prob.

##Models: GIS Only Variables
The GIS only model had a mean squared error of `r round(gis_rf$mse,2)` and and R\textsuperscript{2} `r round(gis_rf$rsq,2)`. Four trophic states were predicted with a total accuracy of 
`r round(gis_ts4_total_acc,2)*100`% and had a kappa coefficient of `r round(gis_ts4_kappa,2)` and two trophic states were correctly predicted `r round(gis_ts2_total_acc,2)*100`% of the time with a kappa coefficient of `r round(gis_ts2_kappa,2)`. 
...
```{r All_Importance, eval=FALSE,results="asis",echo=FALSE, fig.cap="Importance plot for Model 1, shows mean percnetage increase in mean square error.  Higher values of percent increase in mean squared error indicates higher importance.  \\label{fig:All_Importance}",cache=FALSE}
##NEEDS TO BE REDONE
#all_ts4_color<-col_lu[["hexcode"]][col_lu[["variables"]]%in%all_ts4_vars]
importancePlot(all_rf,sumTable(all_ts4[1:100]), data_def=data_def,'gini',size=5)#,aes(colour=all_ts4_color))
```

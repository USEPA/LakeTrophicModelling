...
To account for unique aspects of each lake and characterize lake productivity, we also used various measures of lake morphometry (i.e. depth, volume, fetch, etc.).  As these data are difficult to obtain for large numbers of lakes over broad regions, we used modeled lake morphometry [@hollister2010volume;@hollister_predicting_2011;@lakemorpho2014]. From these prior efforts we inlcuded, Surface Area, Shoreline Length, Shoreline Development, Maximum Depth, Mean Depth, Lake Volume, Maximum Lake Length, Mean Lake Width, Maximum Lake Width, and Fetch. 
...
While random forests are able to handle numerous correlated variables without a decrease in prediction accuracy, one possible downfall to this approach is that the resulting model may be difficult to interpret.  This is a problem often faced in gene selection and in that field, a variable selection method based on random forest has been succesfully applied [@diaz2006gene].  With this method, a minimum set of variables that maximizes model accuracy is provided.  This allows us to start with a full suite of predictor variables but then just use a minimum, more interpretable set of variables to run the final predictive models.  One issue with the approach in `varSelRF` is that becuase of the randomization inherent in random forests it is possible to get some variation in the minimum set of selected variables.  To account for this we repeated `varSelRF` 100 times.  Doing so rather quickly converges on a set of all possible important variables.  

Thus, our modelling work flow is as follows:

1. We use `iterVarSelRF` in our `LakeTrophicModelling` R package identify a minimal list of variables that maximize accuracy the random forest algorithm.  We use this subset of models, the reduced model, to then pass to random forest[@diaz-uriarte2010varSelRF; NEED GITHUB REF]. 
